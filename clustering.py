...
#DEBUG
N=10
MAT=[[random.randint(1,1000) for i in range(N)] for j in range(N)]

#TODO mirror la matriz para que vuelva ha ser una matriz normal (que no quede la mitad de la matriz a 0's)
...
def medoids(M,maxIter,k=3):
    #Select random medoids
    medoids = {}
    randIndexs = random.sample(range(len(M)-1), k)
    ...
    for i in range(maxIter):
        ...
        #Associate
        samplesAssCluster = {}
        for sample in range(len(M)-1):
            distanceList = {}
            for medoid in medoids:
                #Save similarity between the sample and the each of the meloids
                distanceList[M[sample][medoid]] = medoid
            #Assign sample to the meloid which is more similar to
            samplesAssCluster[sample] = distanceList[max(distanceList.keys())] #max becuase the higher the value the closer they are
        for sample in samplesAssCluster:
            print("SAMPLE: "+str(sample)+" Associated group: "+str(samplesAssCluster[sample]))
        #Group
        groups = []
        for i in range(k):
            groups.append([])
        for sample in range(len(M)-1):
            i = 0
            for medoid in medoids:
                if samplesAssCluster[sample] == medoid:
        ...
        for group in groups:
            distanceList = {}
            #Compare each sample with all the samples from the group
            for sample in group:
                sampleDistance = 0
                for sampleToCompare in group:
                    #TODO skip check with itself
                    sampleDistance += M[sample][sampleToCompare]
                distanceList[sampleDistance] = sample
            #Choose the sample which is more similar to the rest of the group
            if len(distanceList) != 0:
                maxDistance = max(distanceList.keys())
                chosenSample = distanceList[maxDistance]
                newMedoids[chosenSample] = maxDistance 
            else:
                newMedoids["EMPTY"] = 0     
        print("NEW_MEDOIDS: "+str(newMedoids))    
        #Compare new medoids with old medoids and choose the ones that had more similarity on their group
        tempMedoids = {}
        returnMedoids = {}
        tempMedoids = {**medoids, **newMedoids}
        #TODO sort dictionary TEMP SOLUTION
        tempMedoids = dict(OrderedDict(sorted(tempMedoids.items(), key=lambda x: x[1]))) 
        print("TEMP_MEDOIDS: "+str(tempMedoids))  
        for i in range(1,k+1):
            index = list(tempMedoids.keys())[-i]
            returnMedoids[index] = tempMedoids[index]
        #Check if we can exit
        if returnMedoids == medoids:
            return returnMedoids                  
        medoids = returnMedoids

printMatrix(MAT)
print(medoids(MAT,10))
...
'''
i = 0
for medoid in tempMedoids:
    if i == k:
        break
    returnMedoids[medoid] = tempMedoids[medoid]  
    i += 1  
'''

'''
#TODO sort dictionary TEMP SOLUTION
newMedoids = dict(OrderedDict(sorted(newMedoids.items(), key=lambda x: x[1])))
print("NEW_MEDOIDS: "+str(newMedoids))
#DICT = SAMPLE:SIMILARITY_FROM_THE_REST
for newMedoid, pastMedoid in zip(newMedoids.keys(), medoids.keys()):
    if newMedoid == "EMPTY" or newMedoids[newMedoid] < medoids[pastMedoid]:
        tempMedoids[pastMedoid] = medoids[pastMedoid]
    else: 
        tempMedoids[newMedoid] = newMedoids[newMedoid] 
#Check if we can exit
'''
